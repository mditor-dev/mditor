...
type Handler = (dom: HTMLElement, maps: KeyMap[]) => () => void;
...
const keymapStrategy: Record<KeymapStrategy, Handler> = {
  ...
  memoryCompose: (function () {
    ...
    return function keymap(dom: HTMLElement, maps: Array<KeyMap>) {
...
// todo keys改为'meta+x' 'ctrl+x'
export function useKeymap(
  maps: Array<KeyMap>,
  dom: HTMLElement,
  strategy: KeymapStrategy = 'memoryCompose',
) {
  const cancel = keymapStrategy[strategy](dom, maps);
  return { cancel };
}
