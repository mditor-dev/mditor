...
export function getFileMenu(): MenuItem {
  ...
  const modifyMd = async (msg: string, operate: 'reset' | 'clear') => {
    const [hook, win] = await getFocusedWinMdStore();
  ...
  return new MenuItem({
    ...
    submenu: [
      ...
      getRecentDocumentsMenu(),
      {
        ...
        click() {
          getFocusedWinMdStore().then(([{ save }]) => save());
        },
      },
      {
        label: '另存为',
        ...
        click() {
          getFocusedWinMdStore().then(([{ save }]) => save('save-as'));
        },
      },
      {
        label: '打开文件位置',
        click() {
          getFocusedWinMdStore().then(([{ state }]) => {
